...
public partial class CSharpCodeParser
{
	...
	public CodeFile? ParseCsFile(string path)
	{
		...
		for (var i = 0; i < count; i++)
		{
			...
			Console.WriteLine($"序号:{i}\t 字符:{currentChar.Replace("\n", "\\n")} " +
			                  ...
			                  $"\r\n临时单词:{_tempWord} ");
			//以 -> 的形式显式领空链
			foreach (var space in _spaces) Console.Write($"->{space.GetType().Name}");
			...
			if (TryAppend2TempWordOrUnProcessWordsOrSemanticInvalidArea(currentChar)) continue;

			//走到这里的时候 待处理单词或者词汇表中已经有内容了 不是字符级别的处理了. 这时候就要看怎么处理这个或者这些词.
			if (TryStartSemanticInvalidArea(currentChar)) continue;
			...
			if (IsSentenceBreakWord(out var currentSentenceBreakWord, out var brokeByTempWordOrUnProcessLastWord))
			{
				...
				if (!ProcessUnProcessWords(currentSentenceBreakWord, brokeByTempWordOrUnProcessLastWord))
				{
					...
					//把这个临时单词放在待处理列表中.
					var unProcessWord = _tempWord.ToString().Trim('\r', '\n', ' ');
					if (unProcessWord.Length > 0) _unProcessWords.Add(unProcessWord);
					//然后重置临时单词
				...
				else
				{
					Console.WriteLine();
					Console.WriteLine(
						$"断句符号处理成功:{currentSentenceBreakWord?.Replace("\r\n", "\\r\\n").Replace("\n", "\\n")}");
	...
	{
		if (TryEndSemanticInvalidArea()) return true;
	...
	private bool TryAppend2TempWordOrUnProcessWordsOrSemanticInvalidArea(string currentChar)
	{
		...
		if (IsWordBreakSymbol(currentChar))
		{
			...
			if (TryAppend2SemanticInvalidArea(currentChar)) return true;
	...
	private string BuildCodeBodyByUnProcessWords()
	{
		...
		{
			if (codeString.Length > 0) codeString.Append(' ');
	...
	private bool IsValidEndWordForCurrent()
	{
		...
		if (_unProcessWords.Count > 0) usingContent = _unProcessWords[^1];

		if (_spaces[^1] is NotesLine && usingContent.EndsWith("\n")) return true;

		if (_spaces[^1] is NotesArea && usingContent.EndsWith("*/")) return true;

		if (_spaces[^1] is Namespace && usingContent.EndsWith("}")) return true;

		if (_spaces[^1] is Class && usingContent.EndsWith("}")) return true;

		if (_spaces[^1] is EnumDefine && usingContent.EndsWith("}")) return true;

		if (_spaces[^1] is SharpLine && usingContent.EndsWith("\n")) return true;

		if (_spaces[^1] is VariableWithStructure && usingContent.EndsWith("}")) return true;

		if (_spaces[^1] is Function && usingContent.EndsWith("}")) return true;

		if (_spaces[^1] is StatementWithStructure && usingContent.EndsWith("}")) return true;
	...
	private bool IsStatementWithStructure(out string? statementType)
	{
		...
		var tagCount = _unProcessWords.Count - 1;
		if (tagCount == 0)
		{
			statementType = null;
			//只有 { 就是普通大括号结构
	...
	private List<Parameter> Convert2Parameters(List<string> words)
	{
		...
		foreach (var current in words)
		{
			...
			//如果类型已经解析完毕了,下面就到名字了.
			if (string.IsNullOrEmpty(currentParam.Name)) currentParam.Name = current;
			//如果当前的参数有默认值但是还没有解析,那么就解析一下
			if (currentParam.DefaultValue != null) currentParam.DefaultValue += current;
		}
