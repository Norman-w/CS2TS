...
public partial class CSharpCodeParser
{
		...
		if (isVariable_AddingWords(out var variablePermission))
		{
			...
			if (variablePermission != null)
			{
				...
				if (cls != null)
				{
					...
					var v = new VariableWithStructure(name, new TypeDefine { Name = type },
						null,
						null,
						null,
						null,
			...
			else
			{
				...
				if (parent is Interface)
				{
					...
					{
						//没有括号的时候就是一个标准的变量定义
						var vn = new VariableNoStructure(null, null, null, null, null, null, null, null, null);
				...
				else
				{
					...
					if (parent is Function)
					{
						...
						if (vrParent.Statements == null) vrParent.Statements = new List<Statement>();
					...
					if (parent is StatementWithStructure)
					{
					...
					if (parent is VariableWithStructure)
					{
	...
	private bool ParseLeftCurlyBraces()
	{
		...
		if (IsIfStatementAddingWords())
		{
			...
			if (parent is Function)
			{
				...
				if (funcParent.Statements == null) funcParent.Statements = new List<Statement>();
		...
		{
			var func = _spaces[^1] as Function;

			if (func.Statements == null) throw new NotImplementedException("没有if 哪里来的else if");
		...
		if (IsElseStatementAddingWords())
		{
			...
			if (func.Statements == null) throw new NotImplementedException("没有if 哪里来的else");
		...
		if (isVariable_StructureAddingWords(out typeName, out permission))
		{
			var parent = _spaces[^1];
			var vbOrInterface =
				new VariableWithStructure(null, null, null, null, null, null, null, null, null, null, null);
			var tagIndex = _unProcessWords.IndexOf(typeName);

			//class 标记后面的一个为类名称
			if (IsInterfaceAddingWords())
				vbOrInterface = new Interface(null, null);
			else if (typeName == "class") vbOrInterface = new Interface(null, null);
			...
			//class 标记后面的一个为类名称
			var maohaoIndex = _unProcessWords.IndexOf(":");

			#region 当有继承的时候

			var extentsWordsCount = _unProcessWords.Count - 1 - maohaoIndex - 1;
			if (maohaoIndex >= 0)
			{
				var extBuilder = new StringBuilder();
				for (var ei = 0; ei < extentsWordsCount; ei++)
				{
					var currentExtWord = _unProcessWords[ei + maohaoIndex + 1];
					if (vbOrInterface.Extends == null) vbOrInterface.Extends = new List<string>();
					...
					extBuilder.Append(currentExtWord.Trim('\r', '\n'));
			...
			{
				var ed = new EnumDefine(null, null, null, null, null);
				...
				ed.Permission = vbOrInterface.Permission == null
			...
			{
				if (parent is Interface) CreateFunction(true);
			}
	...
	private bool ParseRightCurlyBraces()
	{
		if (IsInStatement())
		{
			if (IsValidEndWordForCurrent()) _spaces.RemoveAt(_spaces.Count - 1);
