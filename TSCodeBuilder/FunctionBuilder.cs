...
public class FunctionBuilder
{
	...
	)
	{
		...
		var functionHeaderDefineStringBuilders = new List<StringBuilder>();
		//实际被调用的方法体代码集,如下面的结构
		// private fiiFunc_0(b:number):void{
		//         
		// }
		var functionStructureDefineStringBuilders = new List<StringBuilder>();
		// Enumerable.Repeat(
		// defaultHeaderStringBuilder
		// ,functions.Count).ToList(); 
		//返回值的定义种类。是否有多个函数的返回值类型不一致
		var returnTypeDefines = new Dictionary<string, Parameter>();
		...
		//构建完同名函数的头以后，构建统领函数
		var leaderFunctionCode = BuildLeaderFunction(functions, parent, maxParamsFunction, returnTypeDefines,
			appendPermissionAfterFuncName, tab);
		...
		string tab)
	{
		...
		for (var i = 0; i < maxParamCount; i++)
		{
			...
			for (var j = 0; j < functions.Count; j++)
			{
				...
				else
				{
					...
					var currentInParameterCodeInCurrentFunc =
						ParameterBuilder.ProcessParameter(currentInParameterInCurrentFunc, false);
					if (currentPosParamVersions.ContainsKey(currentInParameterCodeInCurrentFunc) == false)
						currentPosParamVersions.Add(currentInParameterCodeInCurrentFunc,
							currentInParameterInCurrentFunc);
				}
			}

			if (i > 0) leaderFunctionCode.Append(',');
			...
			if (currentPosParamVersions.Count > 1)
			{
				...
				if (currentParamCanBeEmpty) leaderFunctionCode.Append('?');
				leaderFunctionCode.Append(':');
				var typeKindIndex = 0;
				//number|string|undefined
				foreach (var current in currentPosParamVersions)
				{
					if (typeKindIndex > 0) leaderFunctionCode.Append('|');
		...
		{
			var returnTypeIndex = 0;
			foreach (var returnTypeDefine in returnTypeDefines)
			{
				if (returnTypeIndex > 0) leaderFunctionCode.Append('|');
		...
		for (var i = 0; i < functions.Count; i++)
		{
			...
			var hasParam = current.InParameters != null && current.InParameters.Count > 0;
			if (hasParam)
			{
				ifCode.Append("if(");
				for (var j = 0; j < currentFunctionParamCount; j++)
				{
					if (j > 0) ifCode.Append(" && ");
			...
			//调用
			ifCode.Append(tab).Append("\t return this.").Append(maxParamsFunction.Name)
				.Append(appendPermissionAfterFuncName).AppendFormat("_{0}", i)
				.Append("(");
	...
	/// <param name="container4Interface"></param>
	/// <returns></returns>
	public static bool IsExtendFromInterface(Function function, ICodeNode container4Interface)
	{
		...
		if (functions != null && functions.Count == 0)
			return false;
		var functionString = BuildFunctionCode(function, null, container4Interface, null, true, null);
		foreach (var current in functions)
		{
			if (current.Name != function.Name)
				continue;
			var currentFunctionString = BuildFunctionCode(current, null, container4Interface, null, true, null);
	...
	)
	{
		...
		if (function.IsStatic) functionCode.Append("static ");
