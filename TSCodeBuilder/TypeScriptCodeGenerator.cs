...
public class TypeScriptCodeGenerator
{
	...
	{
		var ancestors =
			CodeNode.FindAncestors<IContainer4DefineInterface>(_file, typeof(IContainer4DefineInterface),
				interfaceName);
		//ts中命名空间可以嵌套,类不能嵌套  类会被提权到他所在的命名你空间的儿子级.
		//所以继承的类如果还在某个类当中定义,就不需要那个类
		//比如ClassG定义在 namespaceA namespace B namespaceC classD classE classF当中.
	...
	private void ProcessClass(Class cls)
	{
		...
		if (cls.AccessPermission == AccessModifierPermissionEnum.Public) _currentCode.Append("export ");
	...
	private void ProcessEnum(EnumDefine enumDefine)
	{
		...
		if (enumDefine.Permission == AccessModifierPermissionEnum.Public) _currentCode.Append("export ");
			...
			if (sameNameFunctions.Count > 1)
			{
				if (sameNameFunctions.IndexOf(function) == 0)
				{
					...
					foreach (var group in groupFunctions)
					{
						...
						var appendPermission =
							groupFunctions.Count > 1 && group.Key != AccessModifierPermissionEnum.Public
